...
### If you're sending a PR that adds or changes a data fetcher

There's one extra step beyond a normal PR: register your fetcher in
[`DATA_SOURCES.md`](DATA_SOURCES.md). That file is the single source of
truth for what data we fetch, from where, with what cadence. Adding a
fetcher without registering it would mean it gets forgotten by the
rotation scheduler.

The rule is one sentence: _any PR that adds/modifies/removes a data source
must update DATA_SOURCES.md AND pass `npm run validate:data-sources`
(CI green) before merge_.

The CI runs automatically and points exactly at what's out of sync if it
fails. The PR template shows the per-channel checklist.
