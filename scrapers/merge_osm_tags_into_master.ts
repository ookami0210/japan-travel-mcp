...
interface OsmRecord {
  qid: string;
  osm_ids: string[];
  /** Full OSM tag map, kept by the fetcher for tags outside the lifted set
   *  (amenity / tourism / historic / shop / leisure / natural / …). */
...
async function main(): Promise<void> {
  ...
  for (const a of master.attractions) {
    ...
    // Deterministic category from raw OSM tags (amenity / tourism / shop /
    // leisure / …). Above all a machine-decidable food flag — consumers key
    // meal logic on category === "food". Honest: unmatched tags → no field.
    if (o.raw_tags) {
      const cat = categorizeFromOsmTags(o.raw_tags);
      if (cat) {
        (a as Record<string, unknown>).category = cat;
        category += 1;
