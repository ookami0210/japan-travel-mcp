...
# Rubric v2 — "AI agent usefulness" framing (KJ-confirmed 2026-05-02).
...
def main() -> None:
    ...
    parser.add_argument("--rubric", choices=["v1", "v2"], default="v1",
                        help="v1: original 'canonical asset matching' rubric. "
                             "v2: 'AI-agent usefulness' rubric (KJ-confirmed 2026-05-02). "
...
def write_report(scored: list[dict], path: Path, case_total: int, new_tools: bool, label: str, rubric: str) -> None:
    ...
    lines = [
        ...
        f"Cases: {n}",
        f"Judge: claude-sonnet-4-5 (LLM judge against KJ × Nancy 0/1/2 rubric)",
        "",
        "## Totals",
        "",
