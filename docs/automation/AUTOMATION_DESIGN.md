...
Status: **DRAFT** for KJ review (2026-05-01).
Owner: Archie + KJ.
...
## Design decisions (KJ approved 2026-05-01)

- **State storage**: HF dataset `kjsunada/japan-travel-mcp-data` (no Firestore).
- **Burst trigger**: manual `workflow_dispatch` + PR label `burst-required` —
  Archie recommends labeling, KJ confirms before merge. **No path-based auto-fire.**
...
1. Wait for `burst-required` label on PR (Archie recommends to KJ first)
...
KJ rule: no full auto-fire. Archie recommends, KJ approves.

```
PR opened with scope-affecting change
  → Archie reviews diff
  → Archie comments on PR with recommendation
  → KJ reads recommendation, decides
  → KJ adds 'burst-required' label (or 'burst-not-needed')
  → Merge to main
  → If 'burst-required' label was on the PR: burst-scrape.yml fires
  → Slack notifies completion
```

Implementation: a tiny CI step on PR open that asks Archie to comment.
For now (no cloud Archie): manual recommendation in chat is fine.

## Open questions for KJ
